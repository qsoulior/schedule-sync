...
export function useFetcher(): FetcherContext {
  ...
  onMounted(async () => {
    try {
      ...
      schedulesInfo.value = JSON.parse(responseText, (key, value) =>
  ...
  const filteredSchedulesInfo = computed<ScheduleInfo[]>(() =>
    schedulesInfo.value.filter((schedule) =>
      schedule.group.toLowerCase().includes(searchedGroup.value.toLowerCase())
    )
  );
